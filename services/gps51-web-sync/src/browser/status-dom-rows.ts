...
import {
  ...
  serializeIdentityIndexForBrowser,
...
export function buildIdentityAwareContainerScript(
  identityPayload: ReturnType<typeof serializeIdentityIndexForBrowser>,
...
export function scoreIdentityAwareContainers(
  candidates: Record<string, unknown>[],
): ScrollContainerCandidate[] {
  const scored: ScrollContainerCandidate[] = [];
...
export function buildRowExtractionScript(
...
export async function discoverIdentityAwareContainer(
  page: Page,
  ...
  const debugCandidates: ContainerCandidateDebug[] = candidates.slice(0, 10).map((candidate) => ({
    selector: String(candidate.selector ?? ""),
    ...
    rejectionReason:
      Number(candidate.inventoryIdHits ?? 0) <= 0 &&
...
function buildStatsFromRows(
  ...
  for (const row of rows) {
    ...
    if (resolution.method === "id") idMatches += 1;
    else if (resolution.method === "unique_name") uniqueNameMatches += 1;
    else if (resolution.method === "group_name") groupNameMatches += 1;
    else if (resolution.method === "duplicate_name") duplicateNameRows += 1;
...
export async function extractDeviceRowsFromContainer(
  page: Page,
  containerSelector: string,
  identityIndex: InventoryIdentityIndex,
  portalCount: number | null,
  ...
  const rows: SanitizedRowFields[] = rawRows.map((row) => ({
    text: String(row.text ?? ""),
    title: typeof row.title === "string" ? row.title : null,
    ariaLabel: typeof row.ariaLabel === "string" ? row.ariaLabel : null,
    dataAttributes:
      row.dataAttributes && typeof row.dataAttributes === "object"
        ? (row.dataAttributes as Record<string, string>)
        : {},
  ...
  const rowSamples: SanitizedRowSample[] = [];
...
export function mergeContainerDiscovery(
  legacy: ReturnType<typeof buildScrollContainerDiscoveryResult>,
  identity: Awaited<ReturnType<typeof discoverIdentityAwareContainer>>,
): ScrollContainerCandidate | null {
  if (identity.selectedContainer) return identity.selectedContainer;
