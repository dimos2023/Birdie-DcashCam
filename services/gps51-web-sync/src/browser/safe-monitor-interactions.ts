...
async function clickSafeByLabels(page: Page, labels: string[]): Promise<boolean> {
  for (const label of labels) {
    ...
    for (const locator of candidates) {
      ...
      if (isForbiddenInteraction(text)) continue;
...
async function expandReadOnlyTreeNodes(page: Page): Promise<void> {
  const expandIcons = page.locator(
    ".ivu-tree-arrow, .el-tree-node__expand-icon, .tree-switcher, [class*='expand-icon'], [aria-expanded='false']",
  );
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (parentText && isForbiddenInteraction(parentText)) continue;
    await icon.click({ timeout: 1500 }).catch(() => undefined);
    await page.waitForTimeout(200);
  }
}

export async function performSafeLiveDiscoveryInteractions(page: Page): Promise<void> {
...
export async function performPeriodicLiveDiscoveryInteractions(page: Page): Promise<void> {
  await safeScrollDeviceTree(page);
  await expandReadOnlyTreeNodes(page);
}
