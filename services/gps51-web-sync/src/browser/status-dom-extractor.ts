...
import {
  ...
  type InventoryIdentityIndex,
} from "../gps51/status-dom-identity.js";
import {
  categorizeDomExtractionFailure,
  hasTabSelectionEvidenceForAllTabs,
} from "../gps51/status-dom-failure.js";
import { inferStatusIconMappings } from "../gps51/status-dom-icons.js";
import {
  reconcileDomStatusSets,
  type DomPortalCounts,
  type DomStatusReconciliation,
  type TabPortalSnapshot,
} from "../gps51/status-dom-reconciliation.js";
import {
  buildDatasetSignature,
  isStaleTabDataset,
  portalCountWithinTolerance,
} from "../gps51/status-dom-vxe-core.js";
import { probeAppStateDeviceSets, mergeDomDiscoveryValidationReasons } from "./status-dom-app-state.js";
import {
  discoverTreeScrollContainer,
  type ScrollContainerCandidate,
} from "./status-dom-tree.js";
import {
  ...
  type TabClickDiagnostics,
  type TabSelectionEvidence,
} from "./status-dom-tabs.js";
import {
  discoverIdentityAwareContainer,
  extractDeviceRowsFromContainer,
  mergeContainerDiscovery,
  type ContainerCandidateDebug,
  type RowExtractionResult,
  type SanitizedRowSample,
} from "./status-dom-rows.js";
...
export type TabExtractionResult = {
  ...
  candidateDatasetCounts: Array<{ path: string; count: number }>;
...
export type StatusUiDebug = {
  tabClickDiagnostics: Array<TabClickDiagnostics & { tab: string }>;
  tabSelectionEvidence: Array<TabSelectionEvidence & { tab: string }>;
...
function emptyTabSelectionEvidence(): TabSelectionEvidence {
  return {
    ariaSelected: false,
    activeClass: false,
...
function emptyTabClickDiagnostics(): TabClickDiagnostics {
  return {
    clicked: false,
    matchedLabel: null,
...
async function resetContainerScroll(page: Page, selector: string | null): Promise<void> {
  if (!selector) return;
  await page
    .evaluate(`(() => {
...
type TabRowExtraction = {
  deviceIds: string[];
  extractionMethod: VxeExtractionMethod | "dom_row_scroll";
  selectedContainer: string | null;
  stats: RowExtractionResult["stats"];
  rowSamples: SanitizedRowSample[];
  datasetSignature: string;
  scrollPasses: number;
...
async function extractTabDeviceIds(
  page: Page,
  tab: keyof typeof STATUS_TAB_FALLBACKS,
  identityIndex: InventoryIdentityIndex,
  ...
  let rowResult: TabRowExtraction = containerSelector
    ...
    : {
        ...
        rowSamples: [] as SanitizedRowSample[],
        datasetSignature: buildDatasetSignature(tab, []),
        scrollPasses: 0,
        rejectionReason: "no_device_container" as string | null,
      };

  if (rowResult.deviceIds.length === 0) {
    const vxeResult = await extractVxeDeviceIdsForCurrentTab(
      page,
      identityIndex.deviceIdSet,
      tab,
      portalCount,
    );
  ...
  const acceptedIds =
    ...
    !portalCountWithinTolerance(
      rowResult.deviceIds.length,
      portalSnapshot.before,
      portalSnapshot.after,
      2,
    )
      ? []
      : rowResult.deviceIds;
  ...
  return {
    ...
    extractionMethod:
      staleDatasetRejected || acceptedIds.length === 0
        ? "none"
...
function buildUiDebug(input: {
  ...
  return {
    ...
    datasetSignatures: Object.fromEntries(
      input.tabResults.map((tab) => [tab.tab, tab.datasetSignature]),
    ),
    rejectionReasons: input.rejectionReasons,
    rowSamplesByTab: Object.fromEntries(
      input.tabResults.map((tab) => [tab.tab, tab.rowSamples.slice(0, 10)]),
    ),
  };
}

export async function extractDomStatusSets(
  page: Page,
  inventoryIdsInput: Set<string>,
  options: DomExtractionOptions = {},
  identityIndexInput?: InventoryIdentityIndex,
): Promise<DomStatusExtractionResult> {
  ...
  const selectedTreeContainer = mergeContainerDiscovery(
    legacyContainerDiscovery,
    identityContainerDiscovery,
  );
  const containerSelector = selectedTreeContainer?.selector ?? null;
  const containerDiscoveryReason =
    identityContainerDiscovery.reason ?? legacyContainerDiscovery.reason;

  const tabSequence: Array<keyof typeof STATUS_TAB_FALLBACKS> = ["all", "online", "offline"];
  const tabResults: TabExtractionResult[] = [];
  let previousSignature: string | null = null;
  let previousPortalCount: number | null = null;

  for (const tab of tabSequence) {
    try {
      await resetContainerScroll(page, containerSelector);
      const result = await extractTabDeviceIds(
        page,
        tab,
        identityIndex,
        containerSelector,
  ...
  const tabSnapshots: TabPortalSnapshot[] = tabResults.map((result) => ({
    tab: result.tab,
...
export async function loadInventoryFromMonitorPage(page: Page): Promise<{
...
export async function loadInventoryIdsFromMonitorPage(page: Page): Promise<Set<string>> {
...
export function buildDomStatusSummary(
  extraction: DomStatusExtractionResult,
): Record<string, unknown> {
