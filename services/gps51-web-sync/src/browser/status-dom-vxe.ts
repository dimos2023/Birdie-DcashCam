...
import {
  ...
  type VxeCandidateDiagnostic,
  type VxeExtractionMethod,
...
export function buildVxeExtractionScript(
  inventoryIds: string[],
  tabName: string,
  portalCount: number | null = null,
): string {
  const serializedInventory = JSON.stringify([...inventoryIds]);
  const serializedTab = JSON.stringify(tabName);
  const serializedPortal =
    portalCount == null ? "null" : String(Math.trunc(portalCount));
...
export function buildVxeVirtualScrollScript(inventoryIds: string[]): string {
  const serializedInventory = JSON.stringify([...inventoryIds]);
  return `(() => {
...
function normalizeVxeTabResult(raw: unknown, tabName: string): VxeTabExtractionResult {
  const record = normalizeRecordArray([raw])[0] ?? {};
  const deviceIds = Array.isArray(record.deviceIds)
    ? record.deviceIds.filter((id): id is string => typeof id === "string")
    : [];
  const extractionMethod =
    record.extractionMethod === "vxe_public_api" ||
  ...
  const candidateDatasetCounts = normalizeRecordArray(record.candidateDatasetCounts).map((entry) => ({
    ...
    matchedInventoryCount:
      typeof entry.matchedInventoryCount === "number" ? entry.matchedInventoryCount : 0,
    objectKeys: Array.isArray(entry.objectKeys)
      ? entry.objectKeys.filter((key): key is string => typeof key === "string")
      : [],
  }));
  ...
  return {
    ...
    candidateDatasetCounts,
    datasetSignature:
      typeof record.datasetSignature === "string"
        ? record.datasetSignature
        : buildDatasetSignature(tabName, sortedIds),
  };
}

export async function extractVxeDeviceIdsForCurrentTab(
  page: Page,
  inventoryIds: Set<string>,
  tabName: string,
  portalCount: number | null,
): Promise<VxeTabExtractionResult> {
  ...
  const withinTolerance =
    portalCount == null || Math.abs(result.deviceIds.length - portalCount) <= 2;
  ...
  const normalizedScrollIds = Array.isArray(scrollIds)
    ? scrollIds.filter((id): id is string => typeof id === "string")
    : [];
