...
import {
  analyzeStatusSourceCandidates,
  sanitizeStatusCandidatePayload,
  validateStatusSourceDiscovery,
} from "../gps51/status-source-analyzer.js";
...
function writeSummaryFile(captureDir: string, summary: Record<string, unknown>): void {
  writeFileSync(path.join(captureDir, SUMMARY_FILE), JSON.stringify(summary, null, 2));
}

function logDiscoveryError(log: Logger, err: unknown): void {
...
async function captureTab(
  page: Page,
  tabKey: keyof typeof STATUS_TAB_FALLBACKS,
  observeMs: number,
): Promise<TabCapture> {
...
export async function runStatusSourceDiscovery(): Promise<void> {
  ...
  try {
    ...
    const tabCaptures: TabCapture[] = [];
    const tabSequence: Array<keyof typeof STATUS_TAB_FALLBACKS | "all_return"> = [
      "all",
      "online",
      "offline",
      "all_return",
    ...
    for (const tabKey of tabSequence) {
      ...
      try {
        ...
        log.warn(
          { tab: tabKey, err: err instanceof Error ? err.message : String(err) },
          "Tab capture failed — continuing",
        );
        tabCaptures.push({
          tab: tabKey,
          portalCount: null,
    ...
    const networkCandidates = analyzeStatusSourceCandidates(
      liveCapture.getCaptures(),
      inventoryIds,
    );
    ...
    writeFileSync(
      path.join(config.captureDir, "status-source-candidates.json"),
      JSON.stringify(
