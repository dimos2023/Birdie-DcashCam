...
export async function runPositionRefreshDiscovery(): Promise<void> {
  resetConfigCache();
  ...
  const failureSummary = {
    ...
    requestAction: null,
    requestActions: [] as string[],
    positionLastReceived: false,
    responseLatencyMs: null as number | null,
  ...
  try {
    ...
    let matchedPosition: ParsedPositionLast | null = null;
    let matchedReceivedAt: number | null = null;
    ...
    const waitForPosition = new Promise<void>((resolve) => {
      const detach = attachLiveWebSocketListeners(page, log, {
        onPositionLast: async (position) => {
          if (position.sourceDeviceId !== candidateDeviceId || matchedPosition) return;
    ...
    const position = matchedPosition as ParsedPositionLast | null;
