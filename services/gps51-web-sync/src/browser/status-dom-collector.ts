...
export function extractDeviceIdsFromText(text: string): string[] {
...
export async function collectVisibleDeviceIds(
  page: Page,
  maxScrollPasses = 20,
): Promise<DomDeviceIdCollection> {
  const ids = new Set<string>();
  let scrollPasses = 0;
  ...
  for (let pass = 0; pass < maxScrollPasses; pass++) {
    ...
    for (const id of extractDeviceIdsFromText(bodyText)) ids.add(id);
    ...
    for (const selector of DEVICE_TREE_ROW_SELECTORS) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        for (const id of extractDeviceIdsFromText(text)) ids.add(id);
      }
    }
...
export function reconcileDeviceSets(input: {
  ...
  const overlapInventoryPercent =
    input.inventoryIds.size === 0 ? 0 : Math.round((overlap / input.inventoryIds.size) * 100);
