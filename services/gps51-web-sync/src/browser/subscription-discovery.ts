...
import type { SubscriptionActionCapture } from "./subscription-frame-capture.js";
import {
  clickTab,
  readPortalStatusCounts,
  selectOneDeviceCheckbox,
  subscribeViaMonitorUi,
} from "./monitor-subscription.js";
...
function frameTotals(actions: SubscriptionActionCapture[]): {
  outgoingFrameCount: number;
  incomingFrameCount: number;
  selectedHint: number | null;
} {
  ...
  let selectedHint: number | null = null;
...
function writeDiscoverySummary(captureDir: string, summary: SubscriptionDiscoverySummary): void {
  writeFileSync(
    path.join(captureDir, "subscription-summary.json"),
    JSON.stringify(summary, null, 2),
  );
}

function writeOutgoingFrames(
  captureDir: string,
  actions: SubscriptionActionCapture[],
): void {
  writeFileSync(
    path.join(captureDir, "subscription-outgoing-frames.json"),
    JSON.stringify(
      actions.map((a) => ({
        action: a.action,
...
export async function runSubscriptionDiscovery(): Promise<void> {
  ...
  const actions: SubscriptionActionCapture[] = [];
  ...
  try {
    ...
    actions.push(
      frameCapture.snapshotAction("select_one_device", oneSelected ? 1 : null),
    );
    ...
    const summary: SubscriptionDiscoverySummary = {
      status: "success",
      startedAt,
...
function summarizeKinds(frames: { kind: string }[]): Record<string, number> {
...
function pickRecommendedSubscriptionFrame(
  actions: SubscriptionActionCapture[],
): { action: string; frame: Record<string, unknown>; deviceIdField: string } | null {
  ...
  for (const action of ranked) {
    ...
    if (!outgoing || typeof outgoing.payload !== "object" || outgoing.payload == null) continue;
    const payload = outgoing.payload as Record<string, unknown>;
    const deviceIdField =
      ["deviceids", "deviceIds", "ids", "devicelist", "devices"].find((k) => k in payload) ??
      "deviceids";
...
function printSummary(summary: SubscriptionDiscoverySummary): void {
