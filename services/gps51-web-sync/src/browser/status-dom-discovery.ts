...
import {
  ...
  loadInventoryFromMonitorPage,
  STATUS_DOM_SUMMARY_FILE,
  STATUS_UI_DEBUG_FILE,
} from "./status-dom-extractor.js";
...
function logDiscoveryError(err: unknown): void {
...
export async function runStatusDomDiscovery(): Promise<void> {
  ...
  try {
    ...
    writeFileSync(
      path.join(config.captureDir, "status-all-device-ids.json"),
      JSON.stringify(
    ...
    writeFileSync(
      path.join(config.captureDir, "status-online-device-ids.json"),
      JSON.stringify(
        { count: extraction.reconciliation.onlineIds.length, deviceIds: extraction.reconciliation.onlineIds },
        null,
        2,
      ),
    );
    writeFileSync(
      path.join(config.captureDir, "status-offline-device-ids.json"),
      JSON.stringify(
        {
    ...
    writeFileSync(
      path.join(config.captureDir, "status-dom-debug.json"),
      JSON.stringify(extraction.debug, null, 2),
    );
    writeFileSync(
      path.join(config.captureDir, STATUS_UI_DEBUG_FILE),
    ...
    writeFileSync(
      path.join(config.captureDir, STATUS_DOM_SUMMARY_FILE),
      JSON.stringify(buildDomFailureSummary({ startedAt, error: err }), null, 2),
    );
    throw err;
  } finally {
