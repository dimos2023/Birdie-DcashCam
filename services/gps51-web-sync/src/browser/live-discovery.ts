...
import {
  performPeriodicLiveDiscoveryInteractions,
  performSafeLiveDiscoveryInteractions,
} from "./safe-monitor-interactions.js";
...
function sanitizeRecord(record: Record<string, unknown>): Record<string, unknown> {
  return redactSecrets(record) as Record<string, unknown>;
}

function extractInventoryIds(payload: unknown): Set<string> {
  const parsed = parseInventoryDeviceTree(payload);
  return new Set(parsed.devices.map((d) => d.sourceDeviceId));
}

function splitCandidates(candidates: LiveCandidateMetrics[]): {
  network: LiveCandidateMetrics[];
  websocket: LiveCandidateMetrics[];
} {
  const network: LiveCandidateMetrics[] = [];
  const websocket: LiveCandidateMetrics[] = [];
  for (const candidate of candidates) {
    if (candidate.transportType === "websocket") websocket.push(candidate);
    else network.push(candidate);
  }
  return { network, websocket };
}

export async function runLiveDiscovery(): Promise<void> {
  ...
  try {
    ...
    const analyzed = rawCaptures.map((capture) =>
      analyzeLiveCandidate(capture, inventoryIds, sanitizeRecord),
    );
...
function printLiveSummary(
  summary: ReturnType<typeof buildLiveStateSummary>,
  queryAlarmAnalysis: ReturnType<typeof analyzeQueryAlarmLiveState> | null,
): void {
