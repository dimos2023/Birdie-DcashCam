...
function resolveServicePath(relativePath: string): string {
...
export function ensureCaptureDir(config: AppConfig): void {
...
export function assertStorageStatePermissions(storageStatePath: string): void {
  try {
    const mode = statSync(storageStatePath).mode & 0o777;
    if (process.platform !== "win32" && (mode & 0o077) !== 0) {
      throw new Error(
        `Storage state file is world-readable (${mode.toString(8)}). Restrict permissions to owner only.`,
      );
    }
  } catch (err) {
    if (err instanceof Error && err.message.includes("world-readable")) throw err;
  }
}
