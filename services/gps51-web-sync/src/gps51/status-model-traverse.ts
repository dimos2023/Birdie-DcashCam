...
const DEFAULT_LIMITS: TraversalLimits = { maxDepth: 10, maxObjects: 50_000 };

const SECRET_KEY_PATTERN =
  /token|password|secret|authorization|cookie|session|credential|apikey|api_key|bearer|auth/i;

const STATUS_FIELD_PATTERN =
  /online|offline|status|active|login|connect|state|heart|net|run/i;
...
export function shouldSkipTraversalKey(key: string): boolean {
  if (!key) return true;
  if (SECRET_KEY_PATTERN.test(key)) return true;
  if (key === "window" || key === "document" || key === "location" || key === "navigator") return true;
  return false;
}

export function isSkippableTraversalValue(value: unknown): boolean {
...
export function detectModelSourceFromPath(path: string): TraversedModelHit["source"] {
...
export function extractInventoryIdsFromValue(
  value: unknown,
  ...
  if (isSkippableTraversalValue(value)) return [];
  ...
  if (value instanceof Map) {
    ...
    for (const [key, child] of value.entries()) {
      ...
      if (keyToken) found.add(keyToken);
...
export function findStatusFieldsOnRecord(record: Record<string, unknown>): string[] {
  const fields: string[] = [];
  for (const [key, value] of Object.entries(record)) {
    if (SECRET_KEY_PATTERN.test(key)) continue;
    if (!STATUS_FIELD_PATTERN.test(key)) continue;
    if (
...
export function collectDeviceRecordsFromValue(value: unknown, depth = 0): Record<string, unknown>[] {
  if (depth > 8 || value == null) return [];
  const records: Record<string, unknown>[] = [];

  if (Array.isArray(value)) {
    for (const item of value) records.push(...collectDeviceRecordsFromValue(item, depth + 1));
    return records;
  }
...
export function probeValueForModelHit(
  path: string,
  value: unknown,
  inventorySet: Set<string>,
): TraversedModelHit | null {
  const matchedInventoryIds = [
    ...new Set(extractInventoryIdsFromValue(value, inventorySet)),
  ].sort();
  if (matchedInventoryIds.length === 0) return null;
  ...
  for (const record of deviceRecords.slice(0, 200)) {
    ...
    if (fields.length > 0) hasPerDeviceStatus = true;
    for (const field of fields) statusFields.add(field);
  }
...
export function normalizeBrowserProbeHits(raw: unknown, inventorySet: Set<string>): TraversedModelHit[] {
  if (!Array.isArray(raw)) return [];
  const hits: TraversedModelHit[] = [];
  for (const entry of raw) {
    ...
    const matchedInventoryIds = Array.isArray(record.matchedInventoryIds)
      ? record.matchedInventoryIds.filter((id): id is string => typeof id === "string")
      : [];
    const inventoryFiltered = matchedInventoryIds.filter((id) => inventorySet.has(id));
    if (inventoryFiltered.length === 0) continue;
    hits.push({
      dataPath,
      source:
        record.source === "pinia_store" ||
      ...
      statusFields: Array.isArray(record.statusFields)
        ? record.statusFields.filter((field): field is string => typeof field === "string")
        : [],
...
export function buildVueAppProbeScript(inventorySample: string[]): string {
