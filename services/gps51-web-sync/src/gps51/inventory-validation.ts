...
export function findDuplicateSourceDeviceIds(devices: InventoryDeviceRecord[]): string[] {
...
export function validateInventoryDevices(
  devices: InventoryDeviceRecord[],
  minimum = MIN_INVENTORY_DEVICE_COUNT,
): void {
...
export function assertOrganizationId(organizationId: string): void {
  ...
  if (!postgresUuid.test(normalizedOrganizationId)) {
    throw new InventoryValidationError(
      `ORGANIZATION_ID is not a valid PostgreSQL UUID: ${organizationId}`,
    );
  }
}
