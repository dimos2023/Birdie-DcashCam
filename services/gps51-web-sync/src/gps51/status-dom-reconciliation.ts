...
export function reconcileDomStatusSets(input: {
  ...
  const extractedCounts: DomExtractedCounts = {
  ...
  for (const snapshot of input.tabSnapshots ?? []) {
    ...
    const withinBefore =
      snapshot.portalCountBefore != null &&
      Math.abs(extractedCount - snapshot.portalCountBefore) <= maxTabDelta;
    const withinAfter =
      snapshot.portalCountAfter != null &&
      Math.abs(extractedCount - snapshot.portalCountAfter) <= maxTabDelta;
    if (
      (snapshot.portalCountBefore != null || snapshot.portalCountAfter != null) &&
      !withinBefore &&
      !withinAfter
    ) {
  ...
  if (identicalReason) validationReasons.push(identicalReason);
  ...
  const tabsMissingSelection = (input.tabSnapshots ?? []).filter(
    (snapshot) => snapshot.tabSelectionEvidenceSelected === false,
  );
