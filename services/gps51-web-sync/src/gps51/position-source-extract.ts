import {
  ...
  parsePositionLast,
  type ParsedPositionLast,
} from "./position-last-parser.js";
...
export function resolveCoordinatesFromRecord(
  record: Record<string, unknown>,
): { latitude: number; longitude: number } | null {
  const lat =
    pickNumber(record, ["callat", "calLat", "lat", "latitude", "maplat", "mapLat"]) ?? null;
  const lng =
    pickNumber(record, ["callon", "calLon", "lng", "lon", "longitude", "maplon", "mapLon"]) ??
    null;
  return parseCoordinatePair(lat, lng);
}

export function resolveTimestampsFromRecord(record: Record<string, unknown>): {
  gpsTimestamp: string | null;
  updateTimestamp: string | null;
} {
  const updatedMs =
  ...
  const locatedMs =
    parseEpochMilliseconds(record.validpoistiontime ?? record.validPoistionTime) ?? updatedMs;
  return {
...
export function recommendPositionSource(
  candidates: PositionCandidate[],
  selectedDeviceId: string,
): PositionSourceRecommendation {
  ...
  return {
    ...
    validated: false,
    validationReasons:
      validationReasons.length > 0 ? validationReasons : ["no_valid_position_candidate"],
  };
}

function scorePositionCandidate(candidate: PositionCandidate): number {
  let score = 0;
  if (candidate.fieldPath.includes("cacheMgr.lastPositions")) score += 100;
  if (candidate.updateTimestamp) score += 20;
  if (candidate.gpsTimestamp) score += 10;
  if (candidate.speed != null) score += 5;
  if (candidate.sourcePositionId != null) score += 5;
  if (candidate.fieldPath.includes("mapIconMarkers")) score -= 30;
  return score;
}

export function parsedPositionFromCandidate(candidate: PositionCandidate): ParsedPositionLast | null {
...
export function collectPositionFieldMatches(
  fieldCounts: Record<string, number>,
): Record<string, PositionFieldMatch[]> {
  const out: Record<string, PositionFieldMatch[]> = {};
  for (const field of POSITION_FIELD_PATTERNS) {
    const matches: PositionFieldMatch[] = [];
    const pattern = new RegExp(`(^|\\.)${field}$`, "i");
    for (const [path, count] of Object.entries(fieldCounts)) {
      if (pattern.test(path)) matches.push({ path, count });
    }
...
function walkJsonForDevicePosition(
  value: unknown,
  selectedDeviceId: string,
  path: string,
  out: PositionCandidate[],
  depth = 0,
): void {
...
export function extractNetworkPositionCandidates(
  payloads: Array<{ action: string | null; body: unknown }>,
  selectedDeviceId: string,
): PositionCandidate[] {
  const candidates: PositionCandidate[] = [];
  for (const payload of payloads) {
    const action = payload.action?.toLowerCase() ?? "";
    if (EXCLUDED_NETWORK_ACTIONS.has(action)) continue;
    walkJsonForDevicePosition(payload.body, selectedDeviceId, action || "response", candidates);
  }
  return candidates;
}

export function normalizePositionSourceInspection(raw: unknown): PositionSourceInspection {
  ...
  for (const item of candidatesRaw) {
    ...
    if (!deviceId || !fieldPath) continue;
    candidates.push({
      source,
      fieldPath,
      deviceId,
...
export function buildPositionSourceInspectScript(selectedDeviceId: string): string {
...
export function buildPositionExtractAllScript(inventorySample: string[]): string {
...
export function normalizePositionInventoryExtraction(
  raw: unknown,
  inventoryIds: Set<string>,
): PositionInventoryExtraction {
  const record = isRecord(raw) ? raw : {};
  const sourceRaw = record.source;
  const source =
    sourceRaw === "device_list_tree_nodes" ||
  ...
  const positions: ParsedPositionLast[] = [];
  ...
  for (const item of records) {
    ...
    if (!deviceId || !inventoryIds.has(deviceId)) continue;
    const candidate = candidateFromRecord(
      source ?? "map_component_state",
      typeof item.fieldPath === "string" ? item.fieldPath : `cacheMgr.lastPositions[${deviceId}]`,
      { ...itemRecord, deviceid: deviceId },
      typeof record.componentPath === "string" ? record.componentPath : null,
    );
    if (!candidate) {
  ...
  const missingDeviceIds = [...inventoryIds].filter(
    (id) => !validDeviceIds.includes(id) && !invalidDeviceIds.includes(id),
  );
