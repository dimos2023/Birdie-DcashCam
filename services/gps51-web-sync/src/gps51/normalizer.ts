...
import { DEVICE_ID_KEYS, LOCATION_KEYS, STATUS_KEYS } from "./selectors.js";
...
function parseOnlineStatus(record: Record<string, unknown>): "online" | "offline" | "unknown" {
...
export function normalizeDeviceRecord(
  ...
  source: ParseSource,
): NormalizedGps51Device {
  const sourceDeviceId =
    pickString(record, DEVICE_ID_KEYS) ?? pickString(record, ["sim", "simno", "sim_no"]) ?? "unknown";
  ...
  let accOn: boolean | null = null;
  if (accRaw === 1 || accRaw === true || accRaw === "1") accOn = true;
  if (accRaw === 0 || accRaw === false || accRaw === "0") accOn = false;
...
export function dedupeDevices(devices: NormalizedGps51Device[]): NormalizedGps51Device[] {
  const map = new Map<string, NormalizedGps51Device>();
...
export function collectRecordArrays(payload: unknown): Record<string, unknown>[] {
