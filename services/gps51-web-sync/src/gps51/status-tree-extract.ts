...
export type TreeNodeRecord = {
  ...
  isOnline?: unknown;
  online?: unknown;
  info?: { deviceid?: unknown; isOnline?: unknown; online?: unknown };
...
export function classifyTreeNodeConnectivity(node: TreeNodeRecord | null | undefined): TreeNodeConnectivity {
  if (!node || typeof node !== "object") return "malformed";
...
function readNodeField(node: TreeNodeRecord, key: "isOnline" | "online"): unknown {
  if (Object.prototype.hasOwnProperty.call(node, key)) return node[key];
  if (node.info && Object.prototype.hasOwnProperty.call(node.info, key)) return node.info[key];
  return undefined;
}

export function isTruthyOnline(value: unknown): boolean {
  return value === true || value === 1 || value === "1";
}

export function isFalsyOffline(value: unknown): boolean {
  return value === false || value === 0 || value === "0";
}

export function nodeDeviceId(node: TreeNodeRecord): string | null {
  if (node.deviceid != null) return String(node.deviceid);
  if (node.info?.deviceid != null) return String(node.info.deviceid);
  return null;
}

export function isDeviceTreeNode(node: TreeNodeRecord): boolean {
  return nodeDeviceId(node) != null;
}

export function walkTreeNodes(
  nodes: TreeNodeRecord[] | null | undefined,
): Array<{ deviceId: string; connectivity: TreeNodeConnectivity }> {
  const found: Array<{ deviceId: string; connectivity: TreeNodeConnectivity }> = [];
  const seen = new Set<string>();

  function walk(list: TreeNodeRecord[] | null | undefined): void {
...
export function extractIdsFromTreeNodes(nodes: TreeNodeRecord[]): {
  allDeviceIds: string[];
  onlineDeviceIds: string[];
  offlineDeviceIds: string[];
  malformedNodeCount: number;
  ...
  for (const entry of walked) {
    ...
    if (entry.connectivity === "online") onlineDeviceIds.push(entry.deviceId);
    else offlineDeviceIds.push(entry.deviceId);
  }
...
export function scoreDeviceListCandidate(
  candidate: Omit<DeviceListCandidate, "inventoryOverlapCount" | "inventoryOverlapPercentage">,
  inventoryIds: Set<string>,
): DeviceListCandidate {
  const overlap = candidate.deviceIds.filter((id) => inventoryIds.has(id));
  const inventoryOverlapPercentage =
    inventoryIds.size === 0 ? 0 : Math.round((overlap.length / inventoryIds.size) * 100);
...
export function pickBestDeviceListCandidate(
  candidates: DeviceListCandidate[],
  minOverlapPercent = 99,
): DeviceListCandidate | null {
  const ranked = candidates
    .filter(
      (candidate) =>
        candidate.hasSetCurrentZtree &&
...
export function filterExtractionToInventory(
  extraction: Pick<TreeStatusExtraction, "allDeviceIds" | "onlineDeviceIds" | "offlineDeviceIds">,
  inventoryIds: Set<string>,
): Pick<TreeStatusExtraction, "allDeviceIds" | "onlineDeviceIds" | "offlineDeviceIds" | "counts"> {
...
export function normalizeTreeStatusExtraction(raw: unknown): TreeStatusExtraction {
  const empty: TreeStatusExtraction = {
    source: "device_list_tree_nodes",
    componentPath: null,
  ...
  return {
    ...
    predicateFunction:
      typeof record.predicateFunction === "string"
...
export function reconcileTreeStatusExtraction(input: {
  ...
  if (input.extraction.error) reasons.push(`extraction_error_${input.extraction.error}`);
  if (!input.extraction.componentPath) reasons.push("device_list_component_not_found");
...
export function buildStatusTreeExtractScript(inventorySample: string[]): string {
