...
export function profileStatusFilterDeviceFields(
  records: BootstrapDeviceRecord[],
): DeviceFieldProfile[] {
  return profileDeviceFields(records).filter(
    (profile) => !isExcludedDeviceStatusField(profile.fieldPath),
  );
}

export function discoverStatusFilterFieldCandidates(
  records: BootstrapDeviceRecord[],
  ...
  const candidates: StatusFilterFieldCandidate[] = [];

  for (const profile of profiles) {
    if (!isLowCardinalityProfile(profile, maxDistinct)) continue;
    if (profile.devicesContaining < minDevices) continue;
    if (isExcludedDeviceStatusField(profile.fieldPath)) continue;
    ...
    for (const mapping of buildMappingVariants(distinctValues)) {
      ...
      const unionCoveragePercent =
        records.length === 0 ? 0 : Math.round((unionSet.size / records.length) * 100);
...
export function pickBestStatusFilterFieldCandidate(
  candidates: StatusFilterFieldCandidate[],
): StatusFilterFieldCandidate | null {
  return pickBestFieldCandidate(candidates) as StatusFilterFieldCandidate | null;
}
