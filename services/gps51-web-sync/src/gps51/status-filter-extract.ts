...
const EXCLUDED_FUNCTION_PATTERN =
  /alarm|urgent|message|media|permission|photograph|videolist|poi/i;

export function isExcludedStatusFilterFunction(name: string): boolean {
  return EXCLUDED_FUNCTION_PATTERN.test(name);
}

export function isConnectivityBundleSnippet(snippet: string): boolean {
  const lower = snippet.toLowerCase();
  if (isExcludedStatusFilterFunction(lower)) return false;
  return (
    (lower.includes("selectdevicestatetype") || lower.includes("selecttype")) &&
    (lower.includes("isonline") || lower.includes("lastpositions") || lower.includes(".online"))
  );
}

export function classifyOnlineFromLastPosition(
  lastPosition: { online?: unknown } | null | undefined,
): "online" | "offline" {
  if (lastPosition && lastPosition.online) return "online";
  return "offline";
}

export function extractIdsFromCacheMgr(
  deviceInfos: Record<string, unknown>,
  lastPositions: Record<string, { online?: unknown }>,
...
export function normalizeStatusFilterExtraction(raw: unknown): StatusFilterExtractionResult {
  const empty: StatusFilterExtractionResult = {
    source: null,
    componentPath: null,
    mapping: null,
  ...
  const source =
    record.source === "cache_mgr_last_positions" || record.source === "device_list_tree_nodes"
      ? record.source
      : null;
  ...
  return {
    ...
    predicateFunction:
      typeof record.predicateFunction === "string" ? record.predicateFunction : null,
    allDeviceIds,
    onlineDeviceIds,
    offlineDeviceIds,
    counts: {
...
export function buildStatusFilterExtractScript(): string {
  return `(() => {
    try {
