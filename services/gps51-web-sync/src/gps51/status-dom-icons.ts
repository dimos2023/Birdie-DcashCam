...
export function buildIconSignature(classes: string[]): string {
  return classes
...
export function inferStatusIconMappings(input: {
  rowSamples: Array<SanitizedRowSample & { statusIconClasses?: string[] }>;
  onlineIds: Set<string>;
  offlineIds: Set<string>;
  portalCounts: DomPortalCounts;
}): StatusIconInference {
  const byIcon = new Map<string, { online: number; offline: number }>();
  ...
  for (const row of input.rowSamples) {
    ...
    if (!deviceId || !signature) continue;

    const bucket = byIcon.get(signature) ?? { online: 0, offline: 0 };
    if (input.onlineIds.has(deviceId)) bucket.online += 1;
    if (input.offlineIds.has(deviceId)) bucket.offline += 1;
    byIcon.set(signature, bucket);
  }

  const mappings: StatusIconMapping[] = [];
  for (const [iconSignature, counts] of byIcon) {
    if (counts.online === 0 && counts.offline === 0) continue;
    if (counts.online > 0 && counts.offline > 0) continue;
    mappings.push({
      iconSignature,
