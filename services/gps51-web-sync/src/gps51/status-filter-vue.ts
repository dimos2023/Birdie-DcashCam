...
function inferChangeKind(entry: SanitizedStateEntry): ChangedStatePath["changeKind"] {
  if (entry.type === "array") return "array_length";
  if (entry.type === "object") return "object_keys";
  return "primitive";
}

export function isFrameworkStatePath(path: string): boolean {
  const segments = path.split(".");
  return segments.some((segment) => segment.startsWith("_") || segment.startsWith("$"));
}

export function buildStatusFilterProbeScript(
...
export function normalizeBrowserFilterProbe(raw: unknown): BrowserFilterProbeResult {
  const record = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {};
  const components = Array.isArray(record.components)
    ? record.components.filter((item): item is VueComponentRecord => item != null && typeof item === "object")
    : [];
  const stateSnapshot = Array.isArray(record.stateSnapshot)
    ? record.stateSnapshot.filter((item): item is SanitizedStateEntry => item != null && typeof item === "object")
    : [];
  const functionSources = Array.isArray(record.functionSources)
    ? record.functionSources.filter((item): item is FunctionSourceRecord => item != null && typeof item === "object")
    : [];
    ...
    ? record.collectionHits
        ...
        .map((item) => ({
          ...
          matchedInventoryIds: Array.isArray(item.matchedInventoryIds)
            ? item.matchedInventoryIds.filter((id): id is string => typeof id === "string")
            : [],
