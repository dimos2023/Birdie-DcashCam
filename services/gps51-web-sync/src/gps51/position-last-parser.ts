...
export function parseEpochMilliseconds(value: unknown): number | null {
...
export function parseCoordinatePair(
  ...
  if (Math.abs(latitude) > 90) latitude /= 1_000_000;
  if (Math.abs(longitude) > 180) longitude /= 1_000_000;
...
export function deriveAccOn(
  statusBits: number | null,
  statusText: string | null,
): { accOn: boolean | null; accTextMismatch: boolean } {
  const accFromBit = statusBits != null ? (statusBits & 1) === 1 : null;
  let accFromText: boolean | null = null;
  const text = statusText?.trim() ?? "";
  if (text.startsWith("ACC On")) accFromText = true;
  else if (text.startsWith("ACC Off")) accFromText = false;

  const accTextMismatch =
    accFromBit != null && accFromText != null && accFromBit !== accFromText;

  return { accOn: accFromBit, accTextMismatch };
}

export function derivePositioned(statusBits: number | null, hasValidCoords: boolean): boolean {
  if (!hasValidCoords) return false;
  if (statusBits == null) return hasValidCoords;
  return (statusBits & 2) === 2;
}

function resolveTimestamp(record: Record<string, unknown>): number | null {
  return (
    parseEpochMilliseconds(record.validpoistiontime ?? record.validPoistionTime) ??
    parseEpochMilliseconds(record.updatetime ?? record.updateTime) ??
    parseEpochMilliseconds(record.arrivedtime ?? record.arrivedTime)
  );
}

function buildSanitizedRaw(record: Record<string, unknown>, accTextMismatch: boolean): Record<string, unknown> {
  const raw: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(record)) {
    raw[key] = value;
  }
  for (const key of RAW_ONLY_FIELDS) {
    if (record[key] != null) raw[key] = record[key];
  }
  if (accTextMismatch) {
...
export function parsePositionLast(value: unknown): PositionLastParseResult {
  if (!isRecord(value)) {
  ...
  const coords = parseCoordinatePair(
    pickNumber(value, ["callat", "calLat", "lat", "latitude"]),
    pickNumber(value, ["callon", "calLon", "lng", "lon", "longitude"]),
  );
  if (!coords) {
  ...
  const locatedMs =
    parseEpochMilliseconds(value.validpoistiontime ?? value.validPoistionTime) ??
    updatedMs;
  ...
  const signalStrength =
    rxlevel != null && rxlevel >= 0 && rxlevel <= 100 ? rxlevel : null;
...
export function parseWebsocketMessage(payload: unknown): {
  kind: "positionLast" | "remindMsg" | "ignored";
  data: unknown;
} {
  if (!isRecord(payload)) return { kind: "ignored", data: null };
  if (payload.positionLast != null) return { kind: "positionLast", data: payload.positionLast };
  if (payload.remindMsg != null) return { kind: "remindMsg", data: payload.remindMsg };
  return { kind: "ignored", data: null };
}

export function extractRemindMsgLogFields(value: unknown): { deviceId: string | null; alarmCode: string | null } {
  if (!isRecord(value)) return { deviceId: null, alarmCode: null };
  return {
