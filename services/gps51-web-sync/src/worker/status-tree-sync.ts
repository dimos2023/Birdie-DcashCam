...
import {
  buildStatusTreeExtractScript,
  normalizeTreeStatusExtraction,
  reconcileTreeStatusExtraction,
  type TreeStatusReconciliation,
} from "../gps51/status-tree-extract.js";
import {
...
export async function extractTreeStatusOnPage(
  page: Page,
  inventoryIds: Set<string>,
  config: AppConfig,
): Promise<TreeStatusExtractionResult> {
  ...
  if (fallbackNormalized.source === "device_list_tree_nodes") {
    ...
    const primaryDelta =
      primaryReconciliation.validationReasons.filter((reason) => reason.includes("count_delta")).length;
    const fallbackDelta =
      fallbackReconciliation.validationReasons.filter((reason) => reason.includes("count_delta")).length;
    if (
      fallbackReconciliation.validated ||
...
export function collectInventorySourceFromPayload(payload: unknown): InventorySource {
  return {
...
function writeTreeSyncArtifacts(captureDir: string, summary: Record<string, unknown>): void {
  writeFileSync(path.join(captureDir, STATUS_TREE_SYNC_SUMMARY_FILE), JSON.stringify(summary, null, 2));
}

export function printTreeSyncReport(summary: Record<string, unknown>): void {
  const portalCounts = (summary.portalCounts as ModelPortalCounts | null) ?? {
    all: null,
    online: null,
    offline: null,
  };
  const extractedCounts =
    (summary.extractedCounts as { all: number; online: number; offline: number } | null) ?? {
      all: 0,
      online: 0,
      offline: 0,
    };
...
export async function loadInventoryIdsFromDatabase(
  sb: SupabaseClient,
  accountId: string,
): Promise<Set<string>> {
...
export async function loadInventorySourceFromQueryDeviceTree(
  page: Page,
  config: AppConfig,
...
export async function writeValidatedTreeStatus(
  sb: SupabaseClient,
  config: AppConfig,
  log: Logger,
  inventory: InventorySource,
  result: TreeStatusExtractionResult,
): Promise<{ databaseWrites: number; changedDeviceCount: number }> {
  ...
  const { updated } = await applyTreeStatusBulkUpdates(
    sb,
    config.ORGANIZATION_ID,
    account.id,
    patches,
...
export async function runStatusTreeSync(
  ...
  page?: Page,
  inventoryIds?: Set<string>,
): Promise<{ validated: boolean; databaseWrites: number; changedDeviceCount: number }> {
  ...
  try {
    ...
    if (inventorySet.size < config.GPS51_STATUS_MIN_DEVICES) {
      throw new StatusTreeSyncError(
        `Inventory device count ${inventorySet.size} is below minimum ${config.GPS51_STATUS_MIN_DEVICES}`,
      );
    }
    ...
    if (!result.reconciliation.validated) {
      ...
      if (mode === "sync") {
        throw new StatusTreeSyncError(
          `Tree status extraction failed validation: ${result.reconciliation.validationReasons.join(", ")}`,
        );
      }
    ...
    const previousPortalCounts =
      lastSummary["portalCounts"] && typeof lastSummary["portalCounts"] === "object"
        ? (lastSummary["portalCounts"] as ModelPortalCounts)
        : null;
    const previousComponentPath =
      typeof lastSummary["componentPath"] === "string" ? (lastSummary["componentPath"] as string) : null;
    const previousExtractionError =
      typeof lastSummary["extractionError"] === "string"
        ? (lastSummary["extractionError"] as string)
        : null;
    lastSummary = buildTreeSyncSummary({
      status: "failed",
      mode,
      ...
      malformedNodeCount:
        typeof lastSummary["malformedNodeCount"] === "number"
          ? (lastSummary["malformedNodeCount"] as number)
          : 0,
      errorMessage: message,
    });
    writeTreeSyncArtifacts(config.captureDir, lastSummary);
    if (error instanceof StatusTreeSyncError) throw error;
