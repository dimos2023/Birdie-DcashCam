...
import {
  ...
  extractDomStatusSets,
  loadInventoryIdsFromMonitorPage,
  STATUS_DOM_SUMMARY_FILE,
  STATUS_UI_DEBUG_FILE,
} from "../browser/status-dom-extractor.js";
...
function writeCaptureFiles(
  captureDir: string,
  extraction: Awaited<ReturnType<typeof extractDomStatusSets>>,
): void {
  ...
  writeFileSync(
    path.join(captureDir, "status-all-device-ids.json"),
    JSON.stringify(
      { count: extraction.reconciliation.allIds.length, deviceIds: extraction.reconciliation.allIds },
      null,
      2,
    ),
  );
  writeFileSync(
    path.join(captureDir, "status-online-device-ids.json"),
    JSON.stringify(
      {
  ...
  writeFileSync(
    path.join(captureDir, "status-offline-device-ids.json"),
    JSON.stringify(
      {
  ...
  writeFileSync(
    path.join(captureDir, "status-dom-debug.json"),
    JSON.stringify(redactSecrets(extraction.debug), null, 2),
  );
  writeFileSync(
    path.join(captureDir, STATUS_UI_DEBUG_FILE),
    JSON.stringify(extraction.uiDebug, null, 2),
  );
}

export function printDomSyncReport(
  extraction: Awaited<ReturnType<typeof extractDomStatusSets>>,
...
export async function runDomStatusExtractionOnPage(
  page: Page,
  inventoryIds: Set<string>,
  config: AppConfig,
): Promise<Awaited<ReturnType<typeof extractDomStatusSets>>> {
...
export async function runStatusDomSync(
  sb: SupabaseClient | null,
  config: AppConfig,
  log: Logger,
  mode: StatusDomSyncMode,
  page?: Page,
  inventoryIds?: Set<string>,
): Promise<{ validated: boolean; databaseWrites: number }> {
  let ownsSession = false;
  let cleanup: (() => Promise<void>) | null = null;
  let activePage = page ?? null;
  let inventory = inventoryIds ?? new Set<string>();
  ...
  try {
    ...
    if (inventory.size < config.GPS51_STATUS_MIN_DEVICES) {
      throw new StatusDomSyncError(
        `Inventory device count ${inventory.size} is below minimum ${config.GPS51_STATUS_MIN_DEVICES}`,
      );
    }
    ...
    if (!extraction.reconciliation.validated) {
      ...
      if (mode === "sync") {
        throw new StatusDomSyncError(
          `DOM status extraction failed validation: ${extraction.reconciliation.validationReasons.join(", ")}`,
        );
      }
    ...
    const { updated } = await applyDomStatusUpdates(
      sb,
      config.ORGANIZATION_ID,
      account.id,
      patches,
...
export async function getOrCreateDedicatedStatusPage(
