...
} from "../gps51/inventory-validation.js";
import type {
  InventoryDeviceRecord,
  InventoryDryRunStats,
  InventoryReconciliation,
} from "../gps51/inventory-types.js";
...
import {
  ...
  fetchAccountInventoryDevices,
  reconcileWithoutSync,
  upsertInventoryDevices,
} from "../db/inventory-repository.js";
import {
  finishSyncRun,
  markAccountReauth,
  markAccountSynced,
...
async function captureInventoryFromBrowser(
  config: AppConfig,
  log: ReturnType<typeof createLogger>,
): Promise<{ capture: InventoryCaptureResult; page: Page; cleanup: () => Promise<void> }> {
...
async function verifyGps51Session(page: Page): Promise<void> {
  if (await isReauthRequired(page)) {
    throw new ReauthRequiredError();
  }
}

async function verifySupabase(config: AppConfig): Promise<void> {
...
function writeReconciliation(config: AppConfig, reconciliation: InventoryReconciliation): void {
  writeFileSync(
    path.join(config.captureDir, "inventory-reconciliation.json"),
    JSON.stringify(reconciliation, null, 2),
  );
}

function printDryRunStats(stats: InventoryDryRunStats): void {
...
export async function runInventorySync(mode: InventorySyncMode, config: AppConfig): Promise<void> {
  ...
  try {
    ...
    if (mode === "reconcile") {
      const reconciliation = reconcileWithoutSync(
        summary,
        devices,
        existingRows,
        startedAt,
      ...
      await finishSyncRun(sb, runId, {
        ...
        summary: reconciliation as unknown as Record<string, unknown>,
      });
    ...
    const upsertResult = await upsertInventoryDevices(
      sb,
      config.ORGANIZATION_ID,
      account.id,
      devices,
