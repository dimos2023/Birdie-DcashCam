...
import { incrementStatusRefreshErrors, setStatusChangedDeviceCount, setStatusPortalCounts, setStatusRefreshCounts, setStatusRefreshSuccess, setStatusValidationErrors, } from "./live-sync-metrics.js";
export async function refreshTreeStatusesFromDedicatedPage(context, sb, accountId, config, log, statusPageRef, inventoryIds) {
    try {
        ...
        setStatusPortalCounts(portalCounts.all, portalCounts.online, portalCounts.offline, reconciliation.extractedCounts.all, reconciliation.extractedCounts.online, reconciliation.extractedCounts.offline);
        if (!reconciliation.validated) {
            incrementStatusRefreshErrors();
            setStatusRefreshSuccess(false);
            setStatusValidationErrors(reconciliation.validationReasons);
            setStatusChangedDeviceCount(0);
            log.warn({ reasons: reconciliation.validationReasons }, "Tree status refresh skipped — validation failed");
            return {
                refreshed: false,
                updated: 0,
                changedDeviceCount: 0,
        ...
        setStatusRefreshCounts(reconciliation.extractedCounts.online, reconciliation.extractedCounts.offline, 0);
