...
import { discoverCacheComponentsOnPage, extractOnlineDeviceIdsOnPage, parseCacheRecord, readCachePositionsOnPage, selectDeviceAndWaitForCacheOnPage, } from "../gps51/position-cache-browser.js";
import { isDuplicateCachePosition, isStaleCachePosition, positionTimestampMs, } from "../gps51/position-cache-fingerprint.js";
...
import { buildDeviceLookup, buildLatestUpdateMap, fetchKnownDevices, insertCachePosition, } from "../db/live-position-repository.js";
import { finishSyncRun, markAccountSynced, startSyncRun, } from "../db/repositories.js";
import { collectInventorySourceFromPayload, loadInventorySourceFromQueryDeviceTree, } from "./status-tree-sync.js";
...
export function buildPositionCacheSyncSummary(stats) {
    return {
...
function writePositionCacheArtifacts(captureDir, summary) {
    writeFileSync(path.join(captureDir, POSITION_CACHE_SYNC_SUMMARY_FILE), JSON.stringify(summary, null, 2));
}
export function printPositionCacheSyncReport(summary) {
...
async function collectPositionsFromPage(page, inventorySet, onlineIds, latestPositionMs, config, log, onCurrentDevice) {
    ...
    for (const deviceId of queue) {
        ...
        if (!record) {
            const selection = await selectDeviceAndWaitForCacheOnPage(page, deviceId, config.GPS51_POSITION_WAIT_TIMEOUT_MS);
...
export async function runPositionCacheSync(sb, config, log, mode, page, inventoryIds, options) {
    ...
    try {
        ...
        if (inventory.length === 0) {
            ...
            if (source.duplicateIds.length > 0) {
                throw new PositionCacheSyncError(`Duplicate inventory device IDs found in querydevicestree: ${source.duplicateIds.length}`);
            }
        }
        ...
        const collection = await collectPositionsFromPage(activePage, inventorySet, onlineIds, latestPositionMs, config, log, options?.onCurrentDevice);
        ...
        for (const item of collection.processed) {
            ...
            if (onlineIds.has(item.deviceId))
                onlineDevicesWithPosition += 1;
            else
                offlineDevicesWithPosition += 1;
        ...
        if (mode === "sync" && sb && positionsToWrite.length > 0) {
            ...
            for (const item of positionsToWrite) {
                ...
                if (!device)
                    continue;
                const outcome = await insertCachePosition(sb, config.ORGANIZATION_ID, account.id, syncRunId, device, item.position);
                if (outcome === "inserted") {
                    databaseWrites += 1;
        ...
        const validated = validationReasons.length === 0 &&
        ...
        writePositionCacheArtifacts(config.captureDir, buildPositionCacheSyncSummary(stats));
        if (error instanceof PositionCacheSyncError)
            throw error;
