...
import { buildPositionSourceInspectScript, candidateFromRecord, collectPositionFieldMatches, extractNetworkPositionCandidates, normalizePositionSourceInspection, recommendPositionSource, } from "../gps51/position-source-extract.js";
...
function flattenTreeNodeFields(node) {
    if (!node)
        return {};
    const out = {};
    function walk(value, prefix, depth) {
        ...
        for (const [key, child] of Object.entries(value).slice(0, 60)) {
            if (key.startsWith("_") || key.startsWith("$") || key === "children")
                continue;
...
export async function inspectPositionSourceOnPage(page, selectedDeviceId) {
...
export async function runPositionSourceDiscovery() {
    resetConfigCache();
    ...
    try {
        ...
        let websocketCandidate = null;
        const waitForPosition = new Promise((resolve) => {
            const detach = attachLiveWebSocketListeners(page, log, {
                onPositionLast: async (position) => {
                    if (position.sourceDeviceId !== candidateDeviceId || websocketCandidate)
                        return;
                    websocketCandidate = candidateFromRecord("websocket_positionLast", "positionLast", {
                        ...position.rawPayload,
        ...
        const networkCandidates = extractNetworkPositionCandidates(networkEntries.map((entry) => ({ action: entry.action, body: entry.body })), selectedDeviceId);
        const candidates = [
        ...
        const mergedFieldCounts = { ...beforeFields };
        ...
        writeJson(config.captureDir, POSITION_SOURCE_NETWORK_FILE, redactSecrets({
            selectedDeviceId,
            excludedActions: ["reportmileagedetail", "poibatch"],
            entries: networkEntries,
            websocketPositionLast: websocketCandidate
                ? {
                    latitude: websocketCandidate.latitude,
                    longitude: websocketCandidate.longitude,
                }
                : null,
        }));
