...
function saveActionCaptures(outDir, capture) {
...
export async function runDiscovery() {
    ...
    try {
        ...
        const alarmSummary = alarmCapture?.sanitizedResponse
            ? { kind: "alarm/location", ...summarizeAlarmPayload(alarmCapture.sanitizedResponse) }
            : null;
        ...
        writeJson(outDir, "network-candidates.json", candidates.map((c) => ({
            url: c.url,
            action: c.action,
            method: c.method,
            score: c.score,
            ...
            sampleKeys: c.sanitizedBody && typeof c.sanitizedBody === "object"
                ? Object.keys(c.sanitizedBody).slice(0, 20)
                : [],
        })));
...
function printSchemaSummary(treeResult, domDevices, alarmSummary, capture) {
