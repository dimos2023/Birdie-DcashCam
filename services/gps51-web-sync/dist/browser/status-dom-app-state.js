...
import { appendUniqueReasons, describeEvaluateResultType, normalizeAppStatePathEntries, normalizeRecordArray, normalizeUnknownArray, } from "./status-dom-normalize.js";
const APP_STATE_PATH_PROBE_LIMIT = 40;
const SECRET_VALUE_PATTERN = /token|password|secret|authorization|cookie|session|credential|apikey|api_key|bearer/i;
function isRelevantAppStatePath(path) {
    if (!path || path === "window")
        return false;
    if (/document\.Z__visibilitychange/i.test(path))
        return false;
    if (/^window\.document\./i.test(path))
        return false;
...
export function redactAppStateValue(key, value) {
...
export function findInventoryIdsInUnknownValue(value, inventoryIds, depth = 0) {
    ...
    if (typeof value === "object") {
        const found = new Set();
        const entries = Object.entries(value).slice(0, APP_STATE_OBJECT_KEY_LIMIT);
...
export function logAppStateProbeDiagnostics(diagnostics) {
...
function buildAppStatePathSnapshotScript() {
    return `(() => {
    try {
      var paths = [];
...
function buildReadAppStatePathScript(path) {
...
export async function readAppStateValueAtPath(page, path) {
    return page.evaluate(buildReadAppStatePathScript(path)).catch(() => null);
}
export async function probeAppStateDeviceSets(page, inventoryIds) {
    ...
    const vxeIds = Array.isArray(vxeRecord.deviceIds)
        ? vxeRecord.deviceIds.filter((id) => typeof id === "string")
        : [];
    if (vxeIds.length > 0) {
        const result = {
            used: true,
    ...
    const candidates = normalizeAppStatePathEntries(rawPaths).filter((entry) => isRelevantAppStatePath(entry.path));
    const pathMatches = [];
...
export async function captureAppStateTabDiff(page, inventoryIds, beforePath) {
    if (!beforePath)
