...
export function buildClickStatusTabScript(labels) {
...
export function normalizeTabClickDiagnostics(raw) {
    ...
    return {
        ...
        matchedElementClasses: typeof record.matchedElementClasses === "string" ? record.matchedElementClasses : null,
        clickableAncestorTag: typeof record.clickableAncestorTag === "string" ? record.clickableAncestorTag : null,
        clickableAncestorClasses: typeof record.clickableAncestorClasses === "string" ? record.clickableAncestorClasses : null,
...
export async function clickStatusTabSafely(page, tab) {
...
function resolveTabPortalCount(counts, tab) {
    if (tab === "online")
...
export async function waitForStablePortalCount(page, tab, settleMs) {
    await page.waitForTimeout(settleMs);
    const reads = [];
    for (let i = 0; i < 4; i++) {
        reads.push(await readPortalStatusCounts(page).catch(() => ({
            all: null,
            online: null,
            offline: null,
        })));
...
export async function waitForLoadingToSettle(page, timeoutMs = 8000) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
        const loading = await page
            .locator(".ivu-spin, .el-loading-mask, [class*='loading'], [class*='spinner'], .vxe-loading")
            .first()
            .isVisible()
            .catch(() => false);
        if (!loading)
...
export function buildTabSelectionEvidence(input) {
    const reasons = [];
    if (input.ariaSelected)
        reasons.push("aria_selected");
    if (input.activeClass)
        reasons.push("active_class");
    if (input.clickDiagnostics.classChangedAfterClick)
        reasons.push("class_changed_after_click");
    if (input.portalCountMatches)
        reasons.push("portal_count_matches");
    if (input.rowSignatureChanged)
        reasons.push("row_signature_changed");
    if (input.anotherTabLostActive)
        reasons.push("another_tab_lost_active");
    return {
...
export function buildInspectTabActiveStateScript(labels) {
    const serializedLabels = JSON.stringify(labels);
    return `(() => {
    const labels = ${serializedLabels};
    const activePattern = /active|selected|current|is-active|is-checked|ivu-tabs-tab-active/i;
...
export async function inspectTabActiveState(page, tab) {
    const labels = [...STATUS_TAB_FALLBACKS[tab]];
