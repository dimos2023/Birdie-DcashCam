import { DEVICE_TREE_CONTAINER_SELECTORS, normalizeScrollMetrics, } from "./monitor-dom-safety.js";
import { extractInventoryMatchesFromTexts } from "../gps51/status-dom-matcher.js";
import { describeEvaluateResultType, normalizeRecordArray, normalizeStringArray, } from "./status-dom-normalize.js";
...
export function buildScrollContainerDiscoveryScript(inventorySample, treeSelectors) {
...
export function scoreScrollContainerCandidates(candidates) {
...
export function buildScrollContainerDiscoveryResult(rawCandidates) {
...
export function logContainerDiscoveryDiagnostics(diagnostics) {
...
export function scoreScrollContainer(input) {
    ...
    if (input.clientHeight <= 0 && !input.hasVxeWrapper)
        return 0;
    if (totalHits <= 0)
        return 0;
    let score = 10;
    if (input.matchesTreeSelector)
        score += 15;
    if (input.hasVxeWrapper)
        score += 20;
    ...
    if (input.left != null && input.left < 500)
        score += 10;
    if (input.width != null && input.width < 600)
        score += 5;
    return score;
}
export function isAuthoritativeScrollContainer(candidate) {
    const nameHits = candidate.inventoryNameHits ?? 0;
    return candidate.inventoryIdHits > 0 || nameHits > 0;
}
export function getContainerRejectionReason(candidate) {
    if (!candidate)
        return null;
    const nameHits = candidate.inventoryNameHits ?? 0;
    if (candidate.inventoryIdHits > 0 || nameHits > 0)
        return null;
    return "inventory_id_hits_zero";
}
export function shouldStopTreeExpansion(stalePasses, maxStalePasses = 3) {
    return stalePasses >= maxStalePasses;
}
export async function expandTreeNodesRecursively(page) {
...
export async function discoverTreeScrollContainer(page, inventoryIds) {
...
async function collectVisibleTreeTexts(page, container) {
    const texts = [];
    const bodySnippet = await page.locator("body").innerText().catch(() => "");
    if (bodySnippet)
        texts.push(bodySnippet.slice(0, 50_000));
    ...
    const attributeTexts = normalizeStringArray(await container
        .evaluate(`(root) => {
        try {
        var out = [];
        if (!root) return out;
...
export async function scrollContainerAndCollectIds(page, containerSelector, inventoryIds, expectedCount) {
    ...
    const runPass = async () => {
        ...
        for (let step = 0; step < 250; step++) {
            const metrics = normalizeScrollMetrics(await target
            ...
            passes += 1;
            if (metrics.top == null || metrics.height == null)
                break;
            const clientHeightRaw = await target
                .evaluate("el => (el && typeof el.clientHeight === 'number') ? el.clientHeight : 0")
                .catch(() => 0);
            const clientHeight = typeof clientHeightRaw === "number" && Number.isFinite(clientHeightRaw) ? clientHeightRaw : 0;
            if (clientHeight <= 0)
                break;
            ...
            if (scrollTop + clientHeight >= scrollHeight - 2)
                break;
            await target
                .evaluate("el => { var step = Math.max(50, Math.floor((el.clientHeight || 0) * 0.7)); el.scrollTop = Math.min(el.scrollTop + step, el.scrollHeight); }")
                .catch(() => undefined);
...
export function resolveContainerLocator(page, candidate) {
