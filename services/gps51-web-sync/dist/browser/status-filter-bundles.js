...
export function buildScriptUrlCollectorScript() {
    return `(() => {
    try {
      var urls = [];
...
export function sanitizeBundleUrl(url) {
    return url.split("?")[0] ?? url;
}
export function hashBundleContent(content) {
    return createHash("sha256").update(content).digest("hex").slice(0, 16);
}
export function extractBundleMatches(bundleUrl, bundleFile, content, extraTerms = []) {
...
function findNearbyIdentifier(content, index) {
...
function inferProbablePredicate(snippet, term) {
    ...
    if (term === "querydevicestree")
        return "device_tree_bootstrap";
    if (term === "lastactivetime" || term === "offlinedelay")
        return "timestamp_threshold_candidate";
    return `bundle_term_${term.toLowerCase().replace(/\s+/g, "_")}`;
}
export async function collectBundleMatches(page, bundleDir, extraTerms = []) {
    mkdirSync(bundleDir, { recursive: true });
    const rawUrls = await page.evaluate(buildScriptUrlCollectorScript()).catch(() => []);
    const urls = Array.isArray(rawUrls)
        ? rawUrls.filter((url) => typeof url === "string")
        : [];
    const allMatches = [];
...
function dedupeBundleMatches(matches) {
    const seen = new Set();
    const out = [];
...
export function scoreBundleMatch(match) {
    ...
    let score = 0;
    if (isConnectivityBundleSnippet(match.snippet))
        score += 50;
    if (snippet.includes("selectdevicestatetype"))
        score += 20;
    if (snippet.includes("isonline"))
        score += 15;
    if (snippet.includes("lastpositions"))
        score += 12;
    if (snippet.includes("setcurrentztree"))
        score += 10;
    if (snippet.includes("updateallstate"))
        score += 10;
    if (snippet.includes("deviceinfos"))
        score += 8;
    if (match.probablePredicate.includes("filter"))
        score += 5;
    if (match.matchingTerm === "filter(")
        score += 4;
    if (match.probablePredicate === "status_type_tab_filter")
        score += 4;
    if (identifier.includes("alarm") || identifier.includes("urgent"))
        score -= 50;
    return score;
}
export function pickBestBundleMatch(matches) {
