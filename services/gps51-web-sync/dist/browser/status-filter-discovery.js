...
import { discoverStatusFilterFieldCandidates, pickBestStatusFilterFieldCandidate, } from "../gps51/status-filter-field-profiler.js";
import { buildFilteredCollectionCandidate, categorizeStatusFilterFailure, recommendStatusFilterSource, validateStatusFilterDiscovery, } from "../gps51/status-filter-reconciliation.js";
import { buildStatusFilterProbeScript, compareStateSnapshots, isFrameworkStatePath, normalizeBrowserFilterProbe, } from "../gps51/status-filter-vue.js";
import { clickStatusTabSafely, waitForLoadingToSettle, waitForStablePortalCount, } from "./status-dom-tabs.js";
import { STATUS_TAB_FALLBACKS } from "./monitor-dom-safety.js";
import { buildStatusFilterExtractScript, isExcludedStatusFilterFunction, normalizeStatusFilterExtraction, } from "../gps51/status-filter-extract.js";
...
async function extractStatusFilterIds(page, inventoryIds) {
    ...
    const filterToInventory = (ids) => ids.filter((id) => inventoryIds.has(id));
    return {
        ...extraction,
...
async function probeFilterState(page, inventoryIds) {
    const tabLabels = [
...
async function waitForRowSignatureChange(page, inventoryIds, previousSignature, timeoutMs = 8000) {
...
function buildCollectionCandidates(tab, probe, inventoryIds, portalCount, previousSignatures) {
...
async function captureTabFilterState(page, tab, inventoryIds, previousProbe, previousSignatures, skipClick = false) {
    ...
    const probe = await probeFilterState(page, inventoryIds);
    const changedStatePaths = previousProbe != null
        ? compareStateSnapshots(previousProbe.stateSnapshot, probe.stateSnapshot).filter((change) => !isFrameworkStatePath(change.path))
        : [];
    return {
        tab,
        ...
        changedStatePaths,
        collectionCandidates: buildCollectionCandidates(tab, probe, inventoryIds, portalCount, previousSignatures),
    };
}
function pickBestFunctionCandidate(functions) {
...
function pickBestCollectionPerTab(candidates, tab) {
    return (candidates
...
export async function runStatusFilterDiscovery() {
    ...
    try {
        ...
        for (const step of tabSequence) {
            const captureResult = await captureTabFilterState(page, step.tab, inventoryIds, previousProbe, previousSignatures, step.skipClick);
        ...
        }, [...allFunctions.values()], fieldCandidates.slice(0, 100), collectionCandidates, bundleMatches, validation.onlineDeviceIds, validation.offlineDeviceIds);
        log.info({
            validated: validation.validated,
            recommendedSource: recommendation.source,
...
function writeStatusFilterArtifacts(captureDir, summary, componentState, functions, fieldCandidates, collectionCandidates, bundleMatches, onlineIds, offlineIds) {
    writeFileSync(path.join(captureDir, STATUS_FILTER_SUMMARY_FILE), JSON.stringify(summary, null, 2));
    writeFileSync(path.join(captureDir, STATUS_FILTER_COMPONENT_STATE_FILE), JSON.stringify(componentState, null, 2));
    writeFileSync(path.join(captureDir, STATUS_FILTER_FUNCTIONS_FILE), JSON.stringify(functions, null, 2));
    writeFileSync(path.join(captureDir, STATUS_FILTER_FIELD_CANDIDATES_FILE), JSON.stringify(fieldCandidates, null, 2));
    writeFileSync(path.join(captureDir, STATUS_FILTER_BUNDLE_MATCHES_FILE), JSON.stringify(bundleMatches, null, 2));
    writeFileSync(path.join(captureDir, STATUS_FILTER_ONLINE_IDS_FILE), JSON.stringify({ count: onlineIds.length, deviceIds: onlineIds }, null, 2));
    writeFileSync(path.join(captureDir, STATUS_FILTER_OFFLINE_IDS_FILE), JSON.stringify({ count: offlineIds.length, deviceIds: offlineIds }, null, 2));
}
if (isMain) {
    resetConfigCache();
