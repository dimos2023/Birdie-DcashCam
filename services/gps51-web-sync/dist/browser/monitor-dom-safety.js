...
export function normalizeScrollMetrics(value) {
    ...
    return {
        ...
        height: typeof record.height === "number" && Number.isFinite(record.height)
            ? record.height
            : null,
    };
}
export async function getSafeBoundingBox(locator) {
    try {
...
export function pickTopmostRect(rects) {
    const valid = rects.filter((rect) => rect != null && rect.top != null);
    if (valid.length === 0)
        return null;
    const sorted = [...valid].sort((a, b) => (a.top ?? 0) - (b.top ?? 0));
    return sorted[0] ?? null;
}
export async function readSafeDomRectFromLocator(locator) {
    ...
    `);
        if (evaluated == null)
            return null;
        return normalizeDomRect(evaluated);
    }
    catch {
        return null;
    }
}
export async function scrollDeviceTreeSafely(page) {
...
export async function clickStatusTab(page, tab) {
