import { DEVICE_DETAIL_LABELS, DEVICE_TREE_ROW_SELECTORS, FORBIDDEN_CLICK_TEXT, } from "../gps51/selectors.js";
import { normalizeDeviceRecord } from "../gps51/normalizer.js";
import { scrollDeviceTreeSafely } from "./monitor-dom-safety.js";
export async function scrapeDevicesFromDom(page, maxDevices) {
    const devices = [];
    const seen = new Set();
    for (const selector of DEVICE_TREE_ROW_SELECTORS) {
        ...
        for (let i = 0; i < count && devices.length < maxDevices; i++) {
            ...
            if (!text || isForbiddenInteraction(text))
                continue;
            ...
            devices.push(normalizeDeviceRecord({
                deviceid: deviceId,
...
async function readDetailPanel(page) {
    ...
    if (!bodyText.includes("Device ID") && !bodyText.includes("device id"))
        return null;
    const fields = {};
...
export async function safeScrollDeviceTree(page) {
