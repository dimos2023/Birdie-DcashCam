import { analyzeLiveCandidate, analyzeLiveRecord, collectLiveRecords, rankLiveCandidates, } from "./live-state-analyzer.js";
import { extractLiveDeviceId } from "./live-field-aliases.js";
import { redactSecrets } from "../browser/redaction.js";
export function analyzeStatusSourceCandidates(captures, inventoryIds) {
    const candidates = captures.map((capture) => analyzeLiveCandidate(capture, inventoryIds, (record) => redactSecrets(record)));
...
export function extractOnlineOfflineIdsFromPayload(payload) {
    const online = [];
    const offline = [];
    const all = [];
...
function inferOnlineFromRecord(record) {
...
export function validateStatusSourceDiscovery(input) {
    ...
    const allCount = input.allIds.length > 0
...
export function sanitizeStatusCandidatePayload(payload) {
    return redactSecrets(payload);
}
export function summarizeAnalyzedRecords(payload) {
    let recordCount = 0;
    let withOnlineStatus = 0;
    for (const record of collectLiveRecords(payload)) {
        recordCount += 1;
        const analyzed = analyzeLiveRecord(record, redactSecrets(record));
        if (analyzed.hasOnlineStatus)
            withOnlineStatus += 1;
    }
