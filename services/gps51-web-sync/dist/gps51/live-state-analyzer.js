import { ALARM_URL_HINTS, extractLiveDeviceId, incrementFieldFrequency, parseLiveTimestamp, recordHasCoordinates, recordHasFieldGroup, } from "./live-field-aliases.js";
const TWENTY_FOUR_HOURS_MS = 24 * 60 * 60 * 1000;
function isRecord(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
function looksLikeLiveRecord(record) {
    if (extractLiveDeviceId(record))
        return true;
    if (recordHasCoordinates(record))
        return true;
    if (recordHasFieldGroup(record, "online"))
        return true;
    if (recordHasFieldGroup(record, "speed"))
        return true;
    if (recordHasFieldGroup(record, "acc"))
        return true;
    if (recordHasFieldGroup(record, "timestamp"))
        return true;
    return false;
}
export function extractRootKeys(payload) {
...
export function collectLiveRecords(payload, depth = 0) {
    const results = [];
    ...
    function walk(value, currentDepth) {
        if (currentDepth > 12 || value == null)
            return;
...
export function analyzeLiveRecord(record, sanitized) {
...
export function computeOverlap(deviceIds, inventoryIds) {
    ...
    const overlapPercentage = inventoryIds.size > 0 ? Math.round((overlapCount / inventoryIds.size) * 10000) / 100 : 0;
    return { overlapCount, uniqueDeviceIdCount: unique.size, overlapPercentage };
}
function isAlarmOnlyCandidate(action, url, records) {
    if (action === "queryalarm")
        return true;
    const lower = url.toLowerCase();
    if (ALARM_URL_HINTS.some((hint) => lower.includes(hint)))
        return true;
...
export function analyzeLiveCandidate(capture, inventoryIds, sanitizeRecord) {
    const fieldNameFrequency = {};
    const analyzedRecords = [];
    const rootKeysSet = new Set();
    for (const payload of capture.payloads) {
        for (const key of extractRootKeys(payload))
            rootKeysSet.add(key);
    ...
    const oldest = timestamps.length
        ...
        : null;
    const allRecordsOlderThan24h = timestamps.length > 0 && timestamps.every((d) => now - d.getTime() > TWENTY_FOUR_HOURS_MS);
    const overlap = computeOverlap(analyzedRecords.map((r) => r.deviceId), inventoryIds);
    const base = {
...
export function analyzeQueryAlarmLiveState(payloads, inventoryIds, sanitizeRecord) {
    const analyzed = [];
    ...
    const overlap = computeOverlap(analyzed.map((r) => r.deviceId), inventoryIds);
    ...
    const latest = timestamps.length
        ...
        : null;
    const recordsWithAlarmType = analyzed.filter((r) => "alarmtype" in r.sanitized || "alarmType" in r.sanitized || "type" in r.sanitized).length;
    const recordsWithCoordinates = analyzed.filter((r) => r.hasCoordinates).length;
    const now = Date.now();
    const allOlderThan24h = timestamps.length > 0 && timestamps.every((d) => now - d.getTime() > TWENTY_FOUR_HOURS_MS);
    ...
    const safeForLiveStatus = assessment === "current_positions" &&
...
export function rankLiveCandidates(candidates) {
...
export function buildLiveStateSummary(inventoryDeviceCount, startedAt, finishedAt, observationDurationMs, networkCandidates, websocketCandidates, queryAlarmAnalysis) {
    const rankedCandidates = rankLiveCandidates([...networkCandidates, ...websocketCandidates]);
    const top = rankedCandidates[0] ?? null;
    const validated = top != null &&
        top.rankingScore >= 30 &&
...
export function tryParseJsonPayload(payload) {
...
export function summarizeWebsocketSubscription(payload) {
