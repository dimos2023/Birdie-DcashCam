...
import { normalizePositionInventoryExtraction, parsedPositionFromCandidate, candidateFromRecord, } from "./position-source-extract.js";
import { buildPositionExtractAllScript } from "./position-source-extract.js";
import { parsePositionLast } from "./position-last-parser.js";
export function buildDiscoverCacheComponentsScript() {
    return `(async () => {
    try {
...
export function buildSelectDeviceAndWaitForCacheScript(deviceId, timeoutMs) {
...
export function buildExtractOnlineDeviceIdsScript() {
    return `(async () => {
    try {
      function componentName(vm) {
        if (!vm) return null;
...
export async function readCachePositionsOnPage(page, inventoryIds) {
    ...
    for (const item of Array.isArray(raw?.records)
        ? raw.records
        : []) {
...
export async function discoverCacheComponentsOnPage(page) {
...
export async function extractOnlineDeviceIdsOnPage(page) {
    const script = buildExtractOnlineDeviceIdsScript();
    assertBrowserScriptSafe(script);
    const raw = await page.evaluate(script).catch(() => null);
    const record = raw && typeof raw === "object" ? raw : {};
    const ids = Array.isArray(record.onlineDeviceIds)
        ? record.onlineDeviceIds.filter((item) => typeof item === "string")
        : [];
    return new Set(ids);
}
export async function selectDeviceAndWaitForCacheOnPage(page, deviceId, timeoutMs) {
    ...
    return {
        ...
        record: record.record && typeof record.record === "object" ? record.record : null,
        componentPath: typeof record.componentPath === "string" ? record.componentPath : null,
        error: typeof record.error === "string" ? record.error : null,
    };
}
export function parseCacheRecord(deviceId, record) {
    const candidate = candidateFromRecord("map_component_state", `cacheMgr.lastPositions[${deviceId}]`, { ...record, deviceid: deviceId });
    if (candidate) {
