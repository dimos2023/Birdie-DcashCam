...
export function scoreJsonPayload(body, url, action) {
    let score = 0;
    const lowerUrl = url.toLowerCase();
    const resolvedAction = action ?? extractUrlAction(url);
    if (isAlarmOrAuxiliaryAction(resolvedAction))
        return -25;
    if (isDeviceTreeAction(resolvedAction))
        return 20;
    if (AUTH_URL_HINTS.some((h) => lowerUrl.includes(h)))
        score -= 10;
    if (MAP_URL_HINTS.some((h) => lowerUrl.includes(h)))
        score -= 10;
    ...
    if (DEVICE_ID_KEYS.some((k) => keys.includes(k.toLowerCase())))
        score += 5;
    if (keys.some((k) => /name|plate|device/.test(k)))
        score += 3;
    if (LOCATION_KEYS.some((k) => keys.includes(k.toLowerCase())))
        score += 5;
    if (keys.some((k) => /speed|acc|status|online/.test(k)))
        score += 3;
    return score;
}
/** Generic parser — excludes alarm/auxiliary actions. Prefer parseDeviceTree for inventory. */
export function parseDevicesFromPayload(payload, url) {
    if (url) {
...
export function pickBestDeviceListPayload(candidates) {
